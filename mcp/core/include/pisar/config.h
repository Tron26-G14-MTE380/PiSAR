...
if (!trajectory.empty())
        {
            std::cout << std::fixed << std::setprecision(2); // Optional formatting
        ...
        if (!projected_trajectory.empty())
        {
            std::cout << std::fixed << std::setprecision(2); // Optional formatting
            for (size_t i = 0; i < projected_trajectory.size(); ++i) {
                const auto& pt = projected_trajectory[i];
                std::cout << "(" << pt.x() << ", " << pt.y() << ")";
                if (i != projected_trajectory.size() - 1)
                    std::cout << ", ";
            }
            std::cout << std::endl;
        }